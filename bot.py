...
# ====================== UPGRADED is_site_truly_dead FUNCTION ======================
def is_site_truly_dead(response, price):
    ...
    dead_indicators = [
        ...
        # === HTTP STATUS ERRORS ===
        'site error! status: 400',
        'site error! status: 401',
        'site error! status: 403',
        'site error! status: 404',
        'site error! status: 408',
        'site error! status: 429',
        'site error! status: 500',
        'site error! status: 501',
        'site error! status: 502',
        'site error! status: 503',
        'site error! status: 504',
        'site error! status: 505',
        'site error! status: 520',
        'site error! status: 521',
        'site error! status: 522',
        'site error! status: 523',
        'site error! status: 524',
        'site error! status: 525',
        'site error! status: 526',
        'site error! status: 527',
        'site error! status: 530',
        'http 400', 'http 401', 'http 403', 'http 404',
        'http 408', 'http 429', 'http 500', 'http 502',
        'http 503', 'http 504', 'http 520', 'http 521',
        'http 522', 'http 523', 'http 524', 'http 525',
        ...
        'access denied due to bot activity',
        ...
        'tax amount is empty',
        'del ammount empty',
        'payment base card not supported',
        'checkout_expired',
    ]
    
    # Check if any dead indicator matches
    for ind in dead_indicators:
        if ind in lower:
            return True
    
    # Special check for empty response
...
def format_card_result(status, card, gateway, response, price="-", site="-", bin_info=None, elapsed=0.0):
    ...
    return f"""{h}
<b>━━━━━━━━━━━━━━━━━</b>
<a href='https://t.me/BoobssshopifyBot'>⊀</a> <b>{bs('Card')}</b>
⤷ <code>{card}</code>
...
{PE} <i>{bs('Use /plan to see available plans')}</i>""", buttons=[[pbtn(bs("Upgrade"), url="https://t.me/BoobssshopifyBot")]], emoji_ids=[CE["stop"], CE["stop"], CE["warn"], CE["info"]])
...
def classify_rz_response(rj):
    ...
    rz_charged = [
        ...
        'payment captured', 'payment authorized', 'money received'
    ]
    ...
    rz_declined = [
        ...
        'payment error', 'transaction declined', 'bank declined'
    ]
...
async def _build_status_text():
    ...
    if sys_info.get("error"):
        return f"⌬ <b>𝐄𝐫𝐫𝐨𝐫</b> ↬ <code>❌ {sys_info['error']}</code>\n⌬ <b>𝐁𝐨𝐭 𝐁𝐲</b> ↬ <a href='https://t.me/BoobssshopifyBot'>𝑹@𝒗𝒆𝒏</a>"
    s = sys_info
    msg = (
...
<b>━━━━━━━━━━━━━━━━━</b>
{sl}"""
        kb = [[pbtn(bs("Plans"), data="show_plans"), pbtn(bs("Support"), url="https://t.me/BoobssshopifyBot")],
...
async def plans_cb(event):
    ...
    await styled_send(event.chat_id, plans_text, buttons=[[pbtn(bs("Upgrade"), url="https://t.me/BoobssshopifyBot")]], emoji_ids=[CE["fire"], CE["fire"], CE["crown"]])
...
async def show_plans(event):
    ...
    await styled_reply(event, plans_text, buttons=[[pbtn(bs("Upgrade"), url="https://t.me/BoobssshopifyBot")]], emoji_ids=[CE["fire"], CE["fire"], CE["crown"]])
...
async def info_cmd(event):
    ...
    from database import _load_db
    db_data = _load_db()
    user_doc = db_data["users"].get(str(event.sender_id))
...
async def _check_free_limits(event, uid, plan, is_group):
    ...
    if not is_paid_plan(plan):
        ...
        if used >= FREE_SP_DAILY_LIMIT:
            await styled_reply(event, f"{PE} <b>{bs('Daily Limit')}</b> {used}/{FREE_SP_DAILY_LIMIT}", buttons=[[pbtn(bs("Upgrade"), url="https://t.me/BoobssshopifyBot")]], emoji_ids=[CE["stop"]])
            return False
        cd = get_free_sp_cooldown_remaining(uid)
        if cd > 0:
            await styled_reply(event, f"⚠️ <b>{bs('Wait')} {cd}{bs('s')}</b>", buttons=[[pbtn(bs("Upgrade"), url="https://t.me/BoobssshopifyBot")]])
            return False
    return True
...
async def _handle_plan_assign(event, plan_key):
    ...
    await styled_reply(event, f"""<b>✅ {bs('Plan Updated')}</b>\n<a href='https://t.me/BoobssshopifyBot'>⊀</a> <b>{bs('User')}</b> ↬ <a href='tg://user?id={target_uid}'>{target_name}</a>\n<a href='https://t.me/BoobssshopifyBot'>⊀</a> <b>{bs('Plan')}</b> ↬ {pi['emoji']} <b>{pi['name']}</b>\n<a href='https://t.me/BoobssshopifyBot'>⊀</a> <b>{bs('Duration')}</b> ↬ <code>{pi['duration_days']} {bs('days')}</code>\n<a href='https://t.me/BoobssshopifyBot'>⊀</a> <b>{bs('Expires')}</b> ↬ <code>{expiry_date}</code>""")
    try:
